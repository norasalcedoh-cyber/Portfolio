...
function renderBloque(b) {
  switch (b.tipo) {
    ...
    case 'full':
      return `
        <div class="proyecto__bloque proyecto__bloque--full">
          <img loading="lazy" decoding="async" src="${b.src}" alt="${b.alt}" ${b.height ? `style="height:${b.height}"` : ''} />
    ...
    case 'trio':
      return `
        <div class="proyecto__bloque proyecto__bloque--trio" aria-label="Carrusel de aplicaciones Chimpatía">
          ${b.items.map(item => `
            <div class="proyecto__trio-slide">
              ${item.video
                ? `<video preload="metadata" src="${item.video}" autoplay muted loop playsinline></video>`
                : `<img loading="lazy" decoding="async" src="${item.src}" alt="${item.alt}" />`}
              ${item.caption ? `<p class="proyecto__caption">${item.caption}</p>` : ''}
            </div>
          `).join('')}
