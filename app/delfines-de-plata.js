document.addEventListener('DOMContentLoaded', () => {
  fetch('data/delfines-de-plata.json')
    .then(r => r.json())
    .then(d => init(d))
  ...
  if (nav && d.nav) {
    const prev = d.nav.prev
      ? `<a href="${d.nav.prev.href}" class="proyecto__nav-item proyecto__nav-item--prev">
           ...
           ${d.nav.prev.thumb
             ? d.nav.prev.thumb.endsWith('.mp4')
               ? `<video class="proyecto__nav-thumb" preload="metadata" src="${d.nav.prev.thumb}" autoplay muted loop playsinline></video>`
    ...
    const next = d.nav.next
      ? `<a href="${d.nav.next.href}" class="proyecto__nav-item proyecto__nav-item--next">
           ...
           ${d.nav.next.thumb ? `<img class="proyecto__nav-thumb" src="${d.nav.next.thumb}" alt="${d.nav.next.alt}" />` : ''}
